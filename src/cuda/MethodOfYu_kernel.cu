	...
	MatrixValue* svdMatrices) 
{
		...
		for (int ySp = minIndex3D.y; ySp <= maxIndex3D.y; ySp++)
		{
			for (int xSp = minIndex3D.x; xSp <= maxIndex3D.x; xSp++)
			{
				...
				for (uint i = indexRange.start; i < indexRange.end; i++)
				{
					...
					float dist = length(u);
					if (dist <= dSurfaceParams.anisotropicRadius)
						++numNeighbors;
	...
	// float3 n;
	if (numNeighbors < dSurfaceParams.isolateNumNeighbors)
	{
		ret.a11 = ret.a22 = ret.a33 = MAXUINT;  //int max
		ret.a12 = ret.a13 = ret.a21 = ret.a23 = ret.a31 = ret.a32 = 0;

		ret.maxvalue = 1;
	}
	else if (numNeighbors < dSurfaceParams.minNumNeighbors)
	{
		ret.a11 = ret.a22 = ret.a33 = dSurfaceParams.smoothingRadiusInv;  //int max
		ret.a12 = ret.a13 = ret.a21 = ret.a23 = ret.a31 = ret.a32 = 0;
	...
	SpatialGrid* spatialGrid) 
{
	...
	// float3 pos = smoothedParticles[threadId].pos;

	MatrixValue gMat = svdMatrices[threadId];

	uint3 index3D = getIndex3D(pos, dGridParams.minPos, dGridParams.scGridSize); 	

	float maxdistance = gMat.maxvalue * dSurfaceParams.smoothingRadius;

	uint extent = ceil(maxdistance / dGridParams.scGridSize); 

	uint3 minIndex3D = index3D - extent + 1;
	uint3 maxIndex3D = index3D + extent;

	minIndex3D = clamp(minIndex3D, make_uint3(0, 0, 0), make_uint3(dGridParams.scresolution.x - 1,
		dGridParams.scresolution.y - 1, dGridParams.scresolution.z - 1));
	maxIndex3D = clamp(maxIndex3D, make_uint3(0, 0, 0), make_uint3(dGridParams.scresolution.x - 1,
		dGridParams.scresolution.y - 1, dGridParams.scresolution.z - 1));
	...
	for (int zSp = minIndex3D.z; zSp <= maxIndex3D.z; ++zSp)
	{
		for (int ySp = minIndex3D.y; ySp <= maxIndex3D.y; ++ySp)
		{
			for (int xSp = minIndex3D.x; xSp <= maxIndex3D.x; ++xSp)
			{
				uint3 Index3Dsc = make_uint3(xSp, ySp, zSp);

				uint3 Index3Dsp;

				// Index3Dsp.x = floor(Index3Dsc.x * dGridParams.sptoscScaleInv);
				// Index3Dsp.y = floor(Index3Dsc.y * dGridParams.sptoscScaleInv);
				// Index3Dsp.z = floor(Index3Dsc.z * dGridParams.sptoscScaleInv);

				float3 vPos = getVertexPos(Index3Dsc, dGridParams.minPos, dGridParams.scGridSize);

				Index3Dsp = getIndex3D(vPos, dGridParams.minPos, dGridParams.spGridSize);

				uint index1Dsp = index3DTo1D(Index3Dsp, dGridParams.spresolution);

				// if (!spatialGrid[index1Dsp].inner)
				{
					float3 vertex = getVertexPos(Index3Dsc, dGridParams.minPos, dGridParams.scGridSize);

					if (anisotropicWdistance(pos - vertex, gMat)) 
					{
						uint Index1Dsc = index3DTo1D(Index3Dsc, dGridParams.scresolution);
	...
	ScalarFieldGrid* scalarFieldGrid)
{
	...
	// Index3Dsp.x = round(Index3Dsc.x * dGridParams.sptoscScaleInv);
	// Index3Dsp.y = round(Index3Dsc.y * dGridParams.sptoscScaleInv);
	// Index3Dsp.z = round(Index3Dsc.z * dGridParams.sptoscScaleInv);
