...
void Params::setParams()
{
    ...
    mConfigParams.isSurface = reader.GetBoolean("Config", "IsSurfaceReconstruct", true);
    ...
    mSurfaceParams.minNumNeighbors = reader.GetInteger("Surface", "MinNumNeighbors", 25);
    mSurfaceParams.isolateNumNeighbors = reader.GetInteger("Surface", "IsolateNumNeighbors", 8);
    ...
    mDiffuseParams.smoothingRadius = 1.74*mSimParams.particleSpacing;
    ...
    * 315 / (64 * PI*powf(mDiffuseParams.smoothingRadius, 9));
	...
	// mGridParams.spGridSize =  mDiffuseParams.smoothingRadius;
    mGridParams.spGridSize =  mSimParams.particleSpacing * mGridParams.spScale;
    ...
    mGridParams.minPos_diffuse = mGridParams.minPos + mDiffuseParams.smoothingRadius;
    mGridParams.maxPos_diffuse = mGridParams.maxPos - mDiffuseParams.smoothingRadius;

    mGridParams.minPos = mGridParams.minPos - 2*mSurfaceParams.smoothingRadius;
    mGridParams.maxPos = mGridParams.maxPos + 2*mSurfaceParams.smoothingRadius;

    mGridParams.spresolution = make_uint3(
	((mGridParams.maxPos.x - mGridParams.minPos.x) / mGridParams.spGridSize) + 0.5,
	((mGridParams.maxPos.y - mGridParams.minPos.y) / mGridParams.spGridSize) + 0.5,
	((mGridParams.maxPos.z - mGridParams.minPos.z) / mGridParams.spGridSize) + 0.5);
...
void Params::printInfo()
{
    ...
    std::cout << "particleSpacin " << mSimParams.particleSpacing << std::endl;
    if (mConfigParams.isSurface)
    {
        std::cout << "surfaceSmoothingRadius " << mSurfaceParams.smoothingRadius << std::endl;
        std::cout << "minNumNeighbors " << mSurfaceParams.minNumNeighbors << std::endl;
        std::cout << "lambdaForSmoothed " << mSurfaceParams.lambdaForSmoothed << std::endl;
    ...
    std::cout << "posMin " << mGridParams.minPos_diffuse<< std::endl;
    std::cout << "posMin " << mGridParams.maxPos_diffuse << std::endl;
